...
ls $AGENT_BUILDDIRECTORY
PACKAGE_LIST=($(jq -r 'keys | .[]' "$BUILD_REPOSITORY_LOCALPATH/images/linux/toolcache.json"))
...
if [ -d "$AGENT_TOOLSDIRECTORY/Python" ]; then
   ...
   if [ -n "$python_dirs" ]; then
      PACKAGE_VERSIONS=($(jq -r ".[\"${toolcache_key_value[python]}\"] | .[]" "$BUILD_REPOSITORY_LOCALPATH/images/linux/toolcache.json"))
...
if [ -d "$AGENT_TOOLSDIRECTORY/Ruby" ]; then
   ...
   echo "--------------------------------------"
   if [ -n "$ruby_dirs" ]; then
      PACKAGE_VERSIONS=($(jq -r ".[\"${toolcache_key_value[ruby]}\"] | .[]" "$BUILD_REPOSITORY_LOCALPATH/images/linux/toolcache.json"))
...
if [ -d "$AGENT_TOOLSDIRECTORY/PyPy" ]; then
   ...
   echo "------------------------------------------"
   if [ -n "$pypy_dirs" ]; then
      PACKAGE_VERSIONS=($(jq -r ".[\"${toolcache_key_value[pypy]}\"] | .[]" "$BUILD_REPOSITORY_LOCALPATH/images/linux/toolcache.json"))
